...
with open('./data/featindex.txt') as fin:
    for line in fin:
        ...
        if len(line) > 1:
            f = int(line[0]) - 1
            FIELD_SIZES[f] += 1
#print('field sizes:', FIELD_SIZES) # 一共有26个域(field)，计算出每个field中有多少个属性。
FIELD_OFFSETS = [sum(FIELD_SIZES[:i]) for i in range(len(FIELD_SIZES))] # 计算出每个field的终止坐标，比如:[7,9]，7表示星期这个filed有7个属性，所以终止坐标为7（实际上是0:6，在python切片中冒号后面的坐标不取）；9表示性别，性别只有2个属性，所以终止坐标为9。
INPUT_DIM = sum(FIELD_SIZES) # 所有维度
...
def read_data(file_name):
    ...
    with open(file_name) as fin:
        count = 4096 # max = 3083052
        for line in fin:
            fields = line.strip().split()
            y_i = int(fields[0])
            # X_i = map(lambda x: int(x.split(':')[0]), fields[1:])
            X_i = list(map(lambda x: int(x.split(':')[0]), fields[1:])) # 把每一行对应为1的索引提取出来
            y.append(y_i)
            X.append(X_i)
            count -= 1
            if count<=0: # 这里只取4M条样本
                break
    print('^'*40)
    print(X)
    print('-'*40)
